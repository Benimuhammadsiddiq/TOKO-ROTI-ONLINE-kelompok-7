	...
	<div class="table-responsive">
		<table class="table table-striped">
			...
			<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
				...
				<tr>
					<th scope="row"><?= $no; ?></th>
					<td><img src="image/produk/<?= $row['gambar']; ?>" class="img-fluid" width="100"></td>
					<td><?= $row['nama']; ?></td>
					...
					<td>
                        <input 
                            ...
                            data-price="<?= $row['hrg']; ?>" 
                            data-subtotal-id="subtotal<?= $row['keranjang']; ?>" 
                            data-id="<?= $row['keranjang']; ?>" 
                            value="<?= $row['jml']; ?>" 
                            min="1"
                            required>
                    </td>
                    <td id="subtotal<?= $row['keranjang']; ?>">Rp.<?= number_format($row['hrg'] * $row['jml']); ?></td>
                    <td>
			...
			?>
				<td colspan="7" style="text-align: right; font-weight: bold;" id="grand-total">
